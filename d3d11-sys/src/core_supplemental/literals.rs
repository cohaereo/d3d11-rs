...
macro_rules! s {
    ($s:literal) => {
        $crate::core_supplemental::PCSTR::from_raw(::core::concat!($s, '\0').as_ptr())
    };
}
...
macro_rules! w {
    ($s:literal) => {{
        ...
        const OUTPUT_LEN: usize = $crate::core_supplemental::utf16_len(INPUT) + 1;
        const OUTPUT: &[u16; OUTPUT_LEN] = {
            let mut buffer = [0; OUTPUT_LEN];
            let mut input_pos = 0;
            let mut output_pos = 0;
            while let Some((mut code_point, new_pos)) =
                $crate::core_supplemental::decode_utf8_char(INPUT, input_pos)
            {
                input_pos = new_pos;
        ...
        $crate::core_supplemental::PCWSTR::from_raw(OUTPUT.as_ptr())
    }};
}
...
macro_rules! h {
    ($s:literal) => {{
        const INPUT: &[u8] = $s.as_bytes();
        const OUTPUT_LEN: usize = $crate::core_supplemental::utf16_len(INPUT) + 1;
        static RESULT: $crate::core_supplemental::HSTRING = {
            if OUTPUT_LEN == 1 {
                ...
                const OUTPUT: $crate::core_supplemental::PCWSTR = $crate::core_supplemental::w!($s);
                ...
                unsafe {
                    ::core::mem::transmute::<&HSTRING_HEADER, $crate::core_supplemental::HSTRING>(
                        &HEADER,
                    )
                }
            }
